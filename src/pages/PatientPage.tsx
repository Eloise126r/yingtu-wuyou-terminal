...
export function PatientPage() {
  ...
  const { patient, waitingEstimate, simulateMiniProgramSync, examRoom } = useApp()
  ...
  return (
    <div className="terminal-shell">
      ...
      <main className="terminal-main patient-layout overflow-y-auto">
        {examRoom.status !== 'NORMAL' && <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-3 font-bold text-amber-950"><span className="flex items-center gap-2"><AlertCircle className="text-amber-600" />{examRoom.status === 'DEVICE_FAULT' ? '当前检查设备临时异常' : examRoom.status === 'MAINTENANCE' ? '当前设备临时维护' : '当前检查流程暂时暂停'}</span><span>{examRoom.recoveryEstimate ? `预计可能延长约 ${examRoom.recoveryEstimate} 分钟` : '预计等待时间正在重新评估'}</span></div>}
        ...
        <section className="grid gap-5 lg:grid-cols-[1.18fr_0.82fr]">
          ...
          <div className="panel flex flex-col justify-between p-7"><div className="flex items-center justify-between"><div><p className="eyebrow">动态预测</p><h2 className="mt-1 text-2xl font-black">预计等待时间</h2></div><span className="grid h-14 w-14 place-items-center rounded-2xl bg-cyan-50 text-cyan-600"><Sparkles size={28} /></span></div>{examRoom.status === 'DEVICE_FAULT' && !examRoom.recoveryEstimate ? <div className="my-5"><p className="text-3xl font-black text-amber-800">正在重新评估</p><p className="mt-2 font-bold text-slate-500">受设备状态影响，暂不显示虚假高精度时间</p></div> : <div className="my-5 flex items-baseline gap-2 text-blue-700"><span className="text-[clamp(3.2rem,5vw,5.8rem)] font-black tabular-nums leading-none">{waitingEstimate.min}–{waitingEstimate.max}</span><span className="text-2xl font-black">分钟</span></div>}<div className="grid grid-cols-2 gap-3"><div className="metric-card"><UsersRound />前方患者<strong>{patient.aheadCount} 人</strong></div><div className="metric-card"><Clock3 />当前状态<strong>{examRoom.status === 'NORMAL' ? status.label : '受设备状态影响'}</strong></div></div><p className="mt-4 flex items-start gap-2 text-sm font-semibold leading-6 text-slate-500"><AlertCircle className="mt-0.5 shrink-0 text-amber-500" size={18} />时间为动态区间，急诊优先或检查用时变化时会更新。</p></div>
        </section>

        <section className="grid min-h-0 flex-1 gap-5 lg:grid-cols-[0.92fr_1.08fr]">
          <div className="panel p-6"><div className="flex items-center justify-between"><div><p className="eyebrow">当前检查进度</p><h2 className="mt-1 text-2xl font-black">您现在需要知道</h2></div><BellRing className="text-blue-600" /></div><div className="mt-5 grid grid-cols-5 gap-2">{flow.map((item,index) => <div key={item.label} className="relative text-center"><div className={`mx-auto grid h-10 w-10 place-items-center rounded-full font-black ${item.done ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-400'}`}>{item.done ? <CheckCircle2 size={20} /> : index+1}</div><p className={`mt-2 text-xs font-black ${item.done ? 'text-blue-800' : 'text-slate-400'}`}>{item.label}</p></div>)}</div><div className={`mt-5 rounded-2xl p-5 ${patient.aheadCount === 1 ? 'bg-amber-50 text-amber-950' : 'bg-blue-50 text-blue-950'}`}><p className="flex items-center gap-2 text-lg font-black"><Sparkles size={21} />智能建议</p><p className="mt-2 font-semibold leading-7">{examRoom.status === 'NORMAL' ? advice : '当前候检受检查室运行状态影响，请留意屏幕及工作人员通知。'}</p>{waitingEstimate.max > 20 && <div className="mt-4 flex gap-3"><button className="secondary-action bg-white" onClick={() => navigate('/preparation')}>检查前准备</button><button className="secondary-action bg-white" onClick={() => navigate('/training')}>体位学习</button></div>}</div><button className="mt-4 flex min-h-14 w-full items-center justify-between rounded-2xl bg-slate-50 px-5 font-black text-slate-700" onClick={() => navigate('/waiting')}><span className="flex items-center gap-2"><LogOut />暂离与候检状态</span><ChevronRight /></button></div>
