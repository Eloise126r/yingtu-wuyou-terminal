...
import { currentCall } from '../data/mockData'
      ...
      <div className="relative grid items-center gap-5 lg:grid-cols-[1fr_auto_1fr]">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-lg font-bold text-white/90">请前往检查室，过号请联系工作人员</p>
          </div>
        </div>
        ...
        <div className="flex items-baseline justify-center gap-5 border-y border-white/15 py-3 lg:border-x lg:border-y-0 lg:px-10 lg:py-0">
          <span className="text-[clamp(2.1rem,3.7vw,4.3rem)] font-black tracking-tight">{currentCall.ticketNo}</span>
          <span className="text-[clamp(1.7rem,2.6vw,3rem)] font-black">{currentCall.maskedName}</span>
        </div>

        <div className="flex items-center justify-end gap-3 text-right">
          <div>
            <p className="text-[clamp(1.45rem,2vw,2.1rem)] font-black">{currentCall.room}</p>
            <p className="mt-1 font-semibold text-cyan-50">请按屏幕指引进入</p>
          </div>
