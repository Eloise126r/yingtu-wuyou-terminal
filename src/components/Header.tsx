...
export function Header({ title, subtitle }: HeaderProps) {
  ...
  const staffTimer = useRef<number | undefined>(undefined)
  const location = useLocation()
  const navigate = useNavigate()
  const { resetDemo, elderMode, toggleElderMode, authenticateStaff } = useApp()
  ...
  const startStaffPress = () => { if (!isHome) return; staffTimer.current = window.setTimeout(() => setStaffPinOpen(true), 3000) }
  const cancelStaffPress = () => { if (staffTimer.current) window.clearTimeout(staffTimer.current) }
  ...
  return (
    <><header className="terminal-header">
      <div className="flex min-w-0 items-center gap-4">
        {isHome ? <button className="logo-mark" aria-label="影途无忧" onPointerDown={startStaffPress} onPointerUp={cancelStaffPress} onPointerLeave={cancelStaffPress} onContextMenu={(event) => event.preventDefault()}><Activity strokeWidth={2.6} /></button> : <button className="header-icon-button" onClick={() => navigate(-1)} aria-label="返回上一页"><ArrowLeft /></button>}
    ...
    </header><StaffPinDialog open={staffPinOpen} onClose={() => setStaffPinOpen(false)} onSuccess={(pin) => { const valid = authenticateStaff(pin); if (valid) navigate('/technician'); return valid }} /></>
  )
}
